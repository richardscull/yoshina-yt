...
export class TwitchApiService {
  ...
  public async createInitialCustomReward() {
    try {
      if (!this.twitchClient.user) {
        Log("Twitch", "Cannot create initial custom reward, because no user object was found");
      ...
      if (repeatedReward) {
        ...
        this.twitchClient.customRewardId = repeatedReward.id;
      ...
      Log("Twitch", `Initial custom reward created\nIf you need to update it, visit:\n${coloredLink}`);
  ...
  private async turnOnReward() {
    try {
      ...
      await this.api.channelPoints.updateCustomReward(this.twitchClient.user.id, this.twitchClient.customRewardId, { isEnabled: true });
  ...
  private async turnOffReward() {
    ...
    try {
      ...
      await this.api.channelPoints.updateCustomReward(this.twitchClient.user.id, this.twitchClient.customRewardId, { isEnabled: false });
  ...
  private async rewardCheck() {
    try {
      if (!this.twitchClient.user || !this.twitchClient.customRewardId) {
        Log("Twitch", "Cannot check initial reward for existing because user or custom reward id was not found in twitch cient");

        process.exit(1);
      }

      const reward = await this.api.channelPoints.getCustomRewardById(this.twitchClient.user.id, this.twitchClient.customRewardId!);

      if (!reward) {
        Log("Twitch", "Custom reward needed for script to work properly was not found in twitch dashboard");
