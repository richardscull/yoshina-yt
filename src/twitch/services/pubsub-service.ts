...
export class TwitchPubSubService {
  ...
  private registerHandlers() {
    ...
    this.pubsub.onRedemption(this.twitchClient.user.id, this.onMessage.bind(this));
  }

  private onMessage(msg: PubSubRedemptionMessage) {
    if (!this.twitchClient.user || !this.twitchClient.customRewardId) {
      Log("Twitch", "Can't handle message because user or custom reward id was not found");
    ...
    this.twitchClient.app.youtubeClient.requestsQueue.add({
      message,
      username: userName,
      redeemId: id,
    });
