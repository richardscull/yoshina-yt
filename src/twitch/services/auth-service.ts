...
export class TwitchAuthService {
  ...
  public async getTokensFromCode(code: string) {
    ...
    const data = await exchangeCode(process.env.TWITCH_CLIENT_ID!, process.env.TWITCH_CLIENT_SECRET!, code, process.env.TWITCH_REDIRECT_URL!);
  ...
  public getAuthUrl() {
    ...
    const states = ["channel:manage:redemptions", "channel:read:redemptions", "chat:read", "chat:edit"];
  ...
  private onRefresh(uid: string, { accessToken, refreshToken, expiresIn }: AccessToken) {
